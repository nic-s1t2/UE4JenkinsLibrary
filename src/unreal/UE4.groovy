...
/**
  ...
  * test - the test map to run, eg MainMap_P
...
{
	RunCommand("${UAT} RunUnreal -project=${ProjectFile} -platform=${platform} -build=\"${buildDir}\" -configuration=" + buildConfiguration.name() + " -test=\"DaedalicTestAutomationPlugin.Automation.DaeGauntletTest(ReportPath=${reportDir}, TestName=${test})\" -devices=\"${devicesJson}\" -veryverbose")
	//-scriptdir=\"${EngineDir}/Engine/Binaries/DotNET/AutomationScripts/Daedalic\"
}
